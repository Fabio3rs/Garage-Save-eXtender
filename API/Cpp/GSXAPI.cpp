...
static int(__cdecl *getNewCarGrgForeach)(size_t *i,
                                         GSX::apiCarNotify *out) = nullptr;
static void(__cdecl *setDataToSaveLaterVehPtr)(CVehicle *veh, const char *name,
                                               int size, void *ptr,
                                               bool forceCopyNow) = nullptr;
static void(__cdecl *pushDirectlyToSavedData)(CVehicle *veh, const char *name,
                                              int size, void *ptr) = nullptr;
static int(__cdecl *dataToSaveLaterExists)(CVehicle *veh,
                                           const char *name) = nullptr;
static void(__cdecl *removeToLoadDataVehPtr)(CVehicle *veh,
                                             const char *name) = nullptr;
static void(__cdecl *removeToSaveLaterVehPtr)(CVehicle *veh,
                                              const char *name) = nullptr;
static int(__cdecl *dataToLoadExists)(CVehicle *veh,
                                      const char *name) = nullptr;
static void *(__cdecl *getLoadDataByVehPtr)(CVehicle *veh,
                                            const char *name) = nullptr;
static void *(__cdecl *getLoadDataByVehPtrSz)(CVehicle *veh, const char *name,
                                              int expectingSize) = nullptr;
...
static int(__cdecl *getDataToLoadSize)(CVehicle *veh,
                                       const char *name) = nullptr;
static int(__cdecl *getDataToSaveSize)(CVehicle *veh,
                                       const char *name) = nullptr;

static void(__cdecl *GSX_setScheduledVehicleData)(CVehicle *veh,
                                                  const char *name, int size,
                                                  const void *ptr,
                                                  bool forceCopyNow) = nullptr;
static void(__cdecl *GSX_deleteScheduledVehicleData)(
    CVehicle *veh, const char *name) = nullptr;
static void(__cdecl *GSX_setVehicleData)(CVehicle *veh, const char *name,
                                         int size, const void *ptr) = nullptr;
static void(__cdecl *GSX_deleteVehicleData)(CVehicle *veh,
                                            const char *name) = nullptr;
static uint32_t(__cdecl *GSX_getVehicleData)(CVehicle *veh, const char *name,
                                             uint8_t *ptr,
                                             uint32_t maxsize) = nullptr;
static int(__cdecl *GSX_hasScheduledVehicleData)(CVehicle *veh,
                                                 const char *name) = nullptr;
static int(__cdecl *GSX_hasVehicleData)(CVehicle *veh,
                                        const char *name) = nullptr;
...
auto GSX::setASIPath(const wchar_t *path) -> const wchar_t * {
    if (path == nullptr) {
        return GSXPath;
    }
...
static inline void initFun(T &fun, HMODULE lib, const char *name) {
    auto rawProcAddress = GetProcAddress(lib, name);
    fun = (T)rawProcAddress;
    std::printf("gsx.asi -> %p: %s -> %p\n", lib, name, rawProcAddress);
}
...
static void load() {
    ...
    if (!loaded) {
        ...
        // initFun(::getStoredCarBeforeSpawn, gsxAsi,
        // "getStoredCarBeforeSpawn");
        initFun(::GSX_setScheduledVehicleData, gsxAsi,
                "GSX_setScheduledVehicleData");
        initFun(::GSX_deleteScheduledVehicleData, gsxAsi,
                "GSX_deleteScheduledVehicleData");
        ...
        initFun(::GSX_hasScheduledVehicleData, gsxAsi,
                "GSX_hasScheduledVehicleData");
...
void GSX::setDataToSaveLater(CVehicle *veh, const char *name, int size,
                             void *ptr, bool forceCopyNow) {
    load();
    ::setDataToSaveLaterVehPtr(veh, name, size, ptr, forceCopyNow);
}

void GSX::pushDirectlyToSavedData(CVehicle *veh, const char *name, int size,
                                  void *ptr) {
    load();
...
const char *GSX::getAPIVersionString() { return "0.3a"; }

void GSX::setScheduledVehicleData(CVehicle *veh, const char *name, int size,
...
void GSX::setVehicleData(CVehicle *veh, const char *name, int size,
                         const void *ptr) {
    load();
...
uint32_t GSX::getVehicleData(CVehicle *veh, const char *name, uint8_t *ptr,
                             uint32_t maxsize) {
    load();
