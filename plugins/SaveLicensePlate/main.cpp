...
#include <plugin_sa\game_sa\CMatrix.h>
...
auto RpMaterialSetTexture = injector::cstd<RpMaterial *(RpMaterial *, RwTexture *)>::call<0x0074DBC0>;
auto licensePlateTextureCreate0 = injector::cstd<RwTexture *(RpMaterial *material, const char *a2, char a3)>::call<0x006FE020>;
auto generateLicenseplateMaterial = injector::cstd<RpMaterial *(RpClump *clump, const char *pData, char a3)>::call<0x006FE0F0>;
	...
	if (test->veh)
	{
		...
		case GSX::LOAD_CAR:
		{
			...
			if (plateText != nullptr)
			{
				...
				saveLicensePlateLog << "Before CustomCarPlate_TextureCreate" << std::endl;
				...
				if (CustomCarPlate_TextureCreate(test->veh, info))
				{
					...
					testPlate[8] = 0;
					saveLicensePlateLog << "Plate type " << (int)info->m_nPlateType << std::endl;
		...
		{
			CVehicleModelInfo *info = getVehicleModelInfoByID(test->gameStoredData->model);
...
void onload(int id)
{
	...
	if (!loaded)
	{
		...
		hookPlateDrawOriginal = injector::MakeCALL(0x006D651C, hookPlateDraw).get();
	}
}
