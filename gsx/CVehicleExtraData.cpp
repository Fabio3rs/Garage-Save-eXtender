...
uint64_t CVehicleExtraData::genHash(int model, int handle, const CVector &pos)
{
	...
	crc[0] = crc32FromStdString(std::to_string(px) + std::to_string(handle) + std::to_string(model));
	crc[1] = crc32FromStdString(std::to_string(handle) + std::to_string(model) + GetDateAndTime());
...
{
	return genHash(ptr->m_nModelIndex, CPool_CVehicle__handleOf(getPool(), ptr), ptr->m_matrix->at);
}

CVehicleExtraData::cardata *CVehicleExtraData::vehicleDataRestore(CVehicle *veh, CStoredCar *storedData)
{
	...
	for (auto it = allCars.begin(); it != allCars.end(); /******/)
	{
		...
		*/
		if (/*car.model == storedData->model &&*/ floatTest(car.pos.x, storedData->pos.x) && floatTest(car.pos.y, storedData->pos.y) && floatTest(car.pos.z, storedData->pos.z))
		{
			data.destroy();
...
bool CVehicleExtraData::testGSXReserverdNames(CVehicle *veh, const char *name, cardata &data)
{
	if (name[0] == '_')
	{
		std::string nm = name;
...
extern "C" void __declspec(dllexport) __cdecl setDataToSaveLaterVehPtr(CVehicle *veh, const char *name, int size, const void *ptr, bool forceCopyNow)
{
	if (veh == nullptr)
	{
		return;
...
extern "C" void __declspec(dllexport) __cdecl pushDirectlyToSavedData(CVehicle *veh, const char *name, int size, const void *ptr)
{
	if (veh == nullptr)
	{
		return;
...
extern "C" int __declspec(dllexport) __cdecl dataToSaveLaterExists(CVehicle *veh, const char *name)
{
	if (veh == nullptr)
	{
		return false;
...
extern "C" void __declspec(dllexport) __cdecl removeToLoadDataVehPtr(CVehicle *veh, const char *name)
{
	if (veh == nullptr)
	{
		return;
...
extern "C" void __declspec(dllexport) __cdecl removeToSaveLaterVehPtr(CVehicle *veh, const char *name)
{
	if (veh == nullptr)
	{
		return;
...
extern "C" int __declspec(dllexport) __cdecl dataToLoadExists(CVehicle *veh, const char *name)
{
	if (veh == nullptr)
	{
		std::map<std::string, toLoadData> *toLoad = CVehicleExtraData::inst().beforeSpawned.toLoad;
		if (toLoad)
		{
...
extern "C" __declspec(dllexport) uint32_t __cdecl GSX_getVehicleData(CVehicle * veh, const char* name, uint8_t *ptr, uint32_t maxsize)
{
	if (veh == nullptr)
	{
		std::map<std::string, toLoadData>* toLoad = CVehicleExtraData::inst().beforeSpawned.toLoad;
		if (toLoad)
		{
			...
			if (it != toLoad->end())
			{
				...
				uint32_t copysz = min(obj.second.bytes.size(), maxsize);
				std::copy(obj.second.bytes.data(), obj.second.bytes.data() + copysz, ptr);
...
extern "C" __declspec(dllexport) void* __cdecl getLoadDataByVehPtr(CVehicle *veh, const char *name)
{
	if (veh == nullptr)
	{
		std::map<std::string, toLoadData> *toLoad = CVehicleExtraData::inst().beforeSpawned.toLoad;
...
extern "C" __declspec(dllexport) void* __cdecl getLoadDataByVehPtrSz(CVehicle *veh, const char *name, int expectingSize)
{
	if (veh == nullptr)
	{
		std::map<std::string, toLoadData> *toLoad = CVehicleExtraData::inst().beforeSpawned.toLoad;
...
extern "C" __declspec(dllexport) int __cdecl getDataToLoadSize(CVehicle *veh, const char *name)
{
	if (veh == nullptr)
	{
		std::map<std::string, toLoadData> *toLoad = CVehicleExtraData::inst().beforeSpawned.toLoad;
...
extern "C" __declspec(dllexport) int __cdecl getDataToSaveSize(CVehicle *veh, const char *name)
{
	if (veh == nullptr)
	{
		return -1;
