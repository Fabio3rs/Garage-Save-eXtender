...
extern "C" __declspec(dllexport) int addNotifyTempCallback(
    externalCbFun_t fun) {
    {
...
extern "C" __declspec(dllexport) int getNewCarGrgForeach(size_t *i,
                                                         apiCarNotify *out) {
...
void hashSet(CVehicleExtraData::cardata &data, CVehicle *veh,
             CStoredCar *storedData) {
    {
        ...
        if (data.toLoad["_hash"].bytes.size() == 0) {
            uint64_t chash = CVehicleExtraData::inst().genHash(
                storedData->model,
                CPool_CVehicle__handleOf(CVehicleExtraData::inst().getPool(),
                                         veh),
                storedData->pos);
...
static auto getVehicleModel(CVehicle *veh) {
    uint8_t *tmp = reinterpret_cast<uint8_t *>(veh);
    return *reinterpret_cast<short *>(tmp + 0x22);
}
...
void save(CVehicle *veh, CStoredCar *storedData) {
...

    {
        ...
        if (data.toLoad.size() > 0) {
            car c;
            c.model = getVehicleModel(veh); /*veh->m_nModelIndex;*/
...
typedef injector::function_hooker<injector::scoped_call, 0x0053BCC9, void(void)>
    ngamectscptp_hook;
typedef injector::function_hooker<injector::scoped_call, 0x00618F05, void(void)>
    ngamectscptp_hook2;
typedef injector::function_hooker<injector::scoped_call, 0x0053BE8D, void(void)>
    ngamectscptp_hook3;

void staticHookClearData() { getTempCallbacks().clear(); }
...
void hook() {
...

    {
        injector::make_static_hook<ngamectscptp_hook>(
            [](ngamectscptp_hook::func_type func) {
                staticHookClearData();
                return func();
            });

        injector::make_static_hook<ngamectscptp_hook2>(
            [](ngamectscptp_hook2::func_type func) {
                staticHookClearData();
                return func();
            });

        injector::make_static_hook<ngamectscptp_hook3>(
            [](ngamectscptp_hook3::func_type func) {
                staticHookClearData();
                return func();
            });
    }
